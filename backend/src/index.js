...
const spotifyScopes =
  ...
  'user-read-email user-read-private playlist-read-private playlist-read-collaborative user-read-recently-played';
...
const DEFAULT_RECENTLY_PLAYED_LIMIT = 6;
const MIN_RECENTLY_PLAYED_LIMIT = 1;
const MAX_RECENTLY_PLAYED_LIMIT = 50;
const DEFAULT_NEW_RELEASE_LIMIT = 6;
const MIN_NEW_RELEASE_LIMIT = 1;
const MAX_NEW_RELEASE_LIMIT = 50;
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!recentResponse.ok) {
        ...
        res.status(recentResponse.status === 401 ? 401 : 502).json({
          error: 'Failed to fetch Spotify recently played tracks.',
        });
        return;
      }
      ...
      res.status(502).json({ error: 'Failed to fetch Spotify recently played tracks.' });
    }
  }
);
...
app.get(
  ...
  async (req, res) => {
    ...
    const parsedLimit =
      rawLimit === undefined || rawLimit === '' ? Number.NaN : Number(rawLimit);
    const limitValue = Number.isFinite(parsedLimit) ? parsedLimit : DEFAULT_NEW_RELEASE_LIMIT;
    const limit = Math.max(
      MIN_NEW_RELEASE_LIMIT,
      Math.min(limitValue, MAX_NEW_RELEASE_LIMIT)
    );
    const params = new URLSearchParams({
      limit: limit.toString(),
    });
    ...
    try {
      ...
      if (!releaseResponse.ok) {
        ...
        console.error('Spotify new releases fetch failed:', errorText);
        res.status(releaseResponse.status === 401 ? 401 : 502).json({
          error: 'Failed to fetch Spotify new releases.',
        });
        return;
      }

      const results = await releaseResponse.json();
      res.json(results);
    } catch (error) {
      console.error('Spotify new releases fetch error:', error);
      res.status(502).json({ error: 'Failed to fetch Spotify new releases.' });
    }
  }
);
